...
import ua.nure.yesipov.lang.PhrasesSupplier;
...
public class SpeleologistAgent extends Agent {
    ...
    private PhrasesSupplier phrasesSupplier;

    @Override
    protected void setup() {
        phrasesSupplier = new PhrasesSupplier();
        ...
        private void saveEnvironmentState() {
            ...
            if (reply != null) {
                ...
                if (reply.getPerformative() == ACLMessage.INFORM) {
                    String[] worldResponse = reply.getContent().split(" ");
                    for (int i = 0; i < worldResponse.length; i++) {
                        int state = Integer.parseInt(worldResponse[i]);
                        if (state == 1) {
                            replyBuilder.append(phrasesSupplier.getPhrase(i));
                        }
                    }
                    message = replyBuilder.append("What do I need to do?").toString();
                    step++;
                }
            } else
                block();
